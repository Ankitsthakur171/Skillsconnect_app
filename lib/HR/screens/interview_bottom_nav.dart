...
class _InterviewState extends State<InterviewBottomNav> {
        ...
        body: BlocBuilder<DiscussionBloc, DiscussionState>(
          builder: (context, state) {
            ...
            if (state is DiscussionError) {
              print('🟠 Interview Error Occurred!');
                                            ...
                                            final changed = await Navigator.push<bool>(
                                              ...
                                              MaterialPageRoute(
                                                builder: (_) => BlocProvider.value(
                                                  value: context.read<DiscussionBloc>(), // ✅ same bloc instance
                                                  child: MeetingScreen(meeting: discussion),
                                                ),
                                              ),
                                            );

                                            // ✅ back aate hi reload (sirf jab inner se delete/
                                            // update hua ho)
                                            if (changed == true) {
  ...
  Future<void> _joinInterview(BuildContext context, dynamic discussion) async {
    try {
      ...
      print(
        "🔎 [join] meetingIdRaw=${discussion.id} -> meetingId='$meetingId'",
      );
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // 🔹 Title row (Interview name + job title)
            const Row(
              children: [
                Expanded(
                  child: Text(
            ...
            // 🔹 White details container – time, date, invited, mode
            Container(
            ...
            // 🔹 Bottom buttons (Join / View) – just for layout
            Row(
              children: [
                Expanded(
                  child: ElevatedButton(

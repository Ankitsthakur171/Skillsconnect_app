...
import 'Inner_Interviewdetailpagewithjobid.dart';

class InterviewTabInnerview extends StatefulWidget {
  final ScheduledMeeting meeting;

  const InterviewTabInnerview({super.key, required this.meeting});

  @override
  State<InterviewTabInnerview> createState() => _MeetingScreenState();
}

class _MeetingScreenState extends State<InterviewTabInnerview> {
  ...
  final Set<String> _selectedAttendees = {}; // attendee.applicationId ya unique id
  ...
  final Map<String, int> _sentBumps = {}; // key = application_id, value = +count
...
// ✅ delete click pe kaun-sa row delete ho raha hai, remember
  ...
  void initState() {
    ...
    // ✅ Make a local mutable copy so list updates instantly on delete
    _attendees = List<Attendee>.from(widget.meeting.students);
...
// 🔎 NEW: name ya college par filter (case-insensitive)
                        ...
                        secondChild: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 8),
                              if (meeting.moderatoremail != null && meeting.moderatoremail!.trim().isNotEmpty)
                                Row(
                                  crossAxisAlignment: CrossAxisAlignment.start, // line height fix
                                  children: [
                                    const Image(
                              ...
                              //     // const Text(
                              //     //   "Status",
                              //     //   style: TextStyle(fontSize: 12, color: Color(0xff003840)),
                              //     // ),
                              //     // const Spacer(),
                              //     // Container(
                ...
                Expanded(
                  child: (_q.isNotEmpty && filtered.isEmpty)
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final popupKey = GlobalKey<PopupMenuButtonState<String>>(); // unique per-row
                      final bool isThisMenuOpen =
                          _openMenuAppId == attendee.applicantApplicationId;
                      // ⬇️ put these two lines just above the Expanded(...) inside your itemBuilder
                      final idStr = attendee.applicantApplicationId?.toString() ?? '';
                      final shownCount = (attendee.sendNotificationCount ?? 0) + (_sentBumps[idStr] ?? 0);

                      final String rowStatus = _statusByAppId[attendee.applicantApplicationId] ??
                          (attendee.applicationStatusName?.trim().isNotEmpty == true
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => BlocProvider(
                                    ...
                                    child: InnerInterviewdetailpagewithjobid(
                                      applicationId: appId,
                                      jobId: jobId,
                                      // applicationStatus: attendee.applicationStatusName ?? '',
                                      userId: userId,
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  Checkbox(
                                    value: _selectedAttendees.contains(attendee.applicantApplicationId.toString()), // ✅ String compare
                                    onChanged: (checked) {
                                      setState(() {
                                        if (checked == true) {
                                          _selectedAttendees.add(attendee.applicantApplicationId.toString()); // ✅ String add
                                        } else {
                                          _selectedAttendees.remove(attendee.applicantApplicationId.toString()); // ✅ String remove
                                        }
                                      });
                                    },
                                    shape: RoundedRectangleBorder(
                                    ...
                                    child: Row(
                                      children: [
                                        ...
                                        Container(
                                          ...
                                          child: Text(
                                            '$shownCount', // ⬅️ was attendee.sendNotificationCount.toString()
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          PopupMenuButton<String>(
                                            key: ValueKey(attendee.applicantApplicationId), // unique key per-row

                                            onOpened: () {
                                              setState(() {
                                                _openMenuAppId = attendee.applicantApplicationId; // sirf yeh row "open"
                                              });
                                            },
                                            onCanceled: () {
                                              setState(() {
                                                _openMenuAppId = null; // menu band -> koi row open nahi
                                              });
                                            },
                                            onSelected: (String selected) async {
                                              // UI turant update
                                              setState(() {
                                                _openMenuAppId = null; // select ke baad band
                                              });
                                              ...
                                              if (!confirm) return;
                                              setState(() {
                                                attendee.applicationStatusName = selected;
                                                // agar per-row map use karte ho to yahan bhi set kar sakte ho
                                                // _statusByAppId[attendee.applicantApplicationId] = selected;
                                              });
                                              ...
                                              try {
                                                ...
                                                final body = {
                                                  ...
                                                  "action": selected,
                                                  "application_id_list": [attendee.applicantApplicationId], // 👈 tumhara field
                                                  "process_id": ""
                                                };
                                                ...
                                                if (response.statusCode == 200 && decoded["status"] == true) {
                                                  ...
                                                  ScaffoldMessenger.of(context).showSnackBar(
                                                    SnackBar(content: Text("Failed: ${decoded['message'] ?? 'Unknown error'}"), backgroundColor: Colors.red),
                                                  );
                                                }
                                              } catch (e) {
                                                ScaffoldMessenger.of(context).showSnackBar(
                                              ...
                                              child: Row(
                                                ...
                                                children: [
                                                  Container(
                                                    ...
                                                    decoration: BoxDecoration(
                                                      color: _getStatusColor(rowStatus),
                                                      borderRadius: BorderRadius.circular(40),
                                                    ),
                                                    child: Text(
                                                      rowStatus,
                                                      style: TextStyle(
                                                        ...
                                                        fontFamily: "Inter",
                                                        fontWeight: FontWeight.bold,
                                                        color: _getStatusTextColor(rowStatus),
                                                      ),
                                                    ),
                                                  ),
                                                  const SizedBox(width: 8),
  ...
  PopupMenuItem _buildSelectableMenuItem({
    ...
    return PopupMenuItem(
      ...
      onTap: () {
        ...
        if (index == 0) {
          // 🔥 Send Reminder API
          _sendReminder(widget.meeting.id.toString(), widget.meeting.jobId.toString());
        } else if (index == 2) {
  ...
  Future<void> _deleteMeeting(int meetingId) async {
    ...
    if (confirmed == true) {
      // 🔴 Bloc event fire karna
      context.read<DiscussionBloc>().add(
        TabDeleteMeetingEvent(meetingId: meetingId.toString(), reason: reason),
      );
    }
  }
  ...
  Future<void> _sendReminder(String meetingId, String jobId) async {
    debugPrint("🚀 [_sendReminder] called with meetingId=$meetingId, jobId=$jobId");
    debugPrint("👥 [_sendReminder] selectedAttendees=$_selectedAttendees");

    if (_selectedAttendees.isEmpty) {
      debugPrint("⚠️ [_sendReminder] No attendees selected");
      // ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('auth_token');
      debugPrint("🔑 [_sendReminder] token=${token != null ? 'found' : 'missing'}");

      if (token == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Auth token not found"), backgroundColor: Colors.red),
        );
        return;
      }

      final url = Uri.parse('${BASE_URL}interview-room/send-notification-to-not-joined');
      debugPrint("🌐 [_sendReminder] POST → $url");
      ...
      debugPrint("📦 [_sendReminder] body=${jsonEncode(body)}");
      ...
      debugPrint("📡 [_sendReminder] status=${response.statusCode}");
      debugPrint("📄 [_sendReminder] response=${response.body}");

      final decoded = jsonDecode(response.body);
      if (response.statusCode == 200 && decoded["status"] == true) {
        debugPrint("✅ [_sendReminder] success → updating UI");

        setState(() {
          for (final a in widget.meeting.students) {
            final idStr = a.applicantApplicationId?.toString();
            if (idStr != null && _selectedAttendees.contains(idStr)) {
              _sentBumps.update(idStr, (v) => v + 1, ifAbsent: () => 1);
              debugPrint("🔔 [_sendReminder] incremented count for applicant=$idStr → now=${a.sendNotificationCount}");
            }
          }
        ...
        debugPrint("❌ [_sendReminder] failed → msg=${decoded['msg']}");
        showErrorSnackBar(context, decoded['msg']?.toString() ?? "Internal Server Error");
      }
    } catch (e) {
      debugPrint("💥 [_sendReminder] error=$e");
...
// import '../bloc/InterView_Data/interviewtab_bloc.dart';
// import '../bloc/InterView_Data/interviewtab_event.dart';
// import '../bloc/Interview_Meeting/meeting_bloc.dart';
// import '../bloc/Interview_Meeting/meeting_event.dart';
// import '../model/interview_bottom.dart';
//
// class InterviewTabInnerview extends StatefulWidget {
//
//
//   const InterviewTabInnerview({super.key,});
//
//   @override
//   State<InterviewTabInnerview> createState() => _MeetingScreenState();
// }
...
//           if (state is DiscussionError) {
//             final failure = ApiHttpFailure(
//                 statusCode: null, body: state.message);
//             return OopsPage(failure: failure);
//           }
//
//           if (state is DiscussionLoaded) {
//             final meetings = state.discussions;
//
//             if (meetings.isEmpty) {
//               return const Center(child: Text("No meetings found"));
//             }
//
//             /// ✅ show first meeting detail (ya multiple list bana sakte ho)
...
//                                   detailRow(
//                                     'assets/alarm.png',
//                                     "${meeting.formattedStartTime} to ${meeting
//                                         .formattedEndTime}",
//                                   ),
//                                   const SizedBox(height: 8),
//                                   // detailRow('assets/person.png', meeting.moderator ?? "HR Manager"),
...
//                               String>>(); // unique per-row
//
//                           final String rowStatus = _statusByAppId[attendee
...
//
//                                                     final url = Uri.parse(
//                                                       'https://api.skillsconnect.in/dcxqyqzqpdydfk/mobile/job/dashboard/update-application-status',
//                                                     );
//
//                                                     final body = {
//                                                       "job_id": attendee.jobId,
...
//                     ),
//                     onPressed: () {
//                       if (tempReason.trim().isEmpty) return;
...
//       onTap: () {
//         setState(() => selectedIndex = index);
//         if (index == 3) _deleteMeeting(meetingId);
//       },
//       child: Container(
//         color: bgColor,
//         padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),

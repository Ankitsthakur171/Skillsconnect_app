...
class MediumApi {
  static Future<List<Map<String, dynamic>>> fetchMediums({
    String? authToken,
    String? connectSid,
    int limit = 50,
  }) async {
    try {
      ...
      while (true) {
        final uri = Uri.parse('${ApiConstantsStu.subUrl}master/medium/list');
        ...
        final body = jsonEncode({'medium_name': '', 'limit': limit, 'offset': offset});
        ...
        for (final item in data) {
          out.add({
            'id': item['id'],
            'medium_name': (item['medium_name'] ?? '').toString(),
            'raw': item,
          });
        }
